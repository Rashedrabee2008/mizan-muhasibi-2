// ============================================================
// الميزان 14.0.0 - app-extras-v2.js (نسخة مصححة)
...
console.log('🚀 تحميل app-extras-v2.js - النسخة المصححة');
...
(function initBarcodeScanner() {
    ...
    window.openBarcodeScanner = function() {
        loadQuagga(function() {
            if (!window.Quagga) {
                if (typeof showToast === 'function') showToast('⚠️ لم يتم تحميل الماسح', 'error');
                return;
            }
            ...
            const html = '<button class="modal-close" onclick="closeBarcodeScanner()">&times;</button>' +
                ...
                '</div>' +
                '<div style="text-align:center;color:#A89070;font-size:12px;margin-bottom:12px;">💡 وجّه الكاميرا نحو الباركود</div>' +
                '<div id="barcodeResult" style="background:#0D0D0D;border-radius:8px;padding:12px;text-align:center;font-weight:900;color:#C9A94E;min-height:40px;display:flex;align-items:center;justify-content:center;">⏳ جاري البحث...</div>' +
    ...
    window.handleBarcodeScanned = function(code) {
        ...
        if (product) {
            ...
            if (productSelect) {
                ...
                if (typeof showToast === 'function') showToast('✅ ' + product.name, 'success');
                setTimeout(function() { if (typeof addSaleItem === 'function') addSaleItem(); }, 200);
            }
        } else {
            if (typeof showToast === 'function') showToast('⚠️ لم يوجد منتج: ' + code, 'warning');
        }
    };

    // إضافة زر المسح - مرة واحدة فقط
    function addScanButton() {
        if (document.getElementById('scanBarcodeBtn')) return;
    ...
    function tryAdd() {
        attempts++;
        addScanButton();
        if (attempts < 10 && !document.getElementById('scanBarcodeBtn')) {
            setTimeout(tryAdd, 1500);
        }
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() { setTimeout(tryAdd, 2000); });
    } else {
...
(function initSmartAlerts() {
    ...
    window.showSmartAlerts = function() {
        ...
        let html = '<button class="modal-close" onclick="closeModal()">&times;</button><h3>🔔 التنبيهات الذكية</h3>';

        if (alerts.length === 0) {
            html += '<div class="empty-state"><i class="fas fa-check-circle" style="color:#2D8F5E;"></i><span style="color:#2D8F5E;">كل شيء تمام!</span></div>';
        } else {
            alerts.forEach(function(alert) {
                const colors = { 'danger': '#E06060', 'warning': '#E6A830' };
    ...
    function updateAlertsBadge() {
        ...
        if (!badge && alerts.length > 0) {
            ...
            badge.style.cssText = 'background:#E06060;border:none;color:#fff;font-size:11px;cursor:pointer;padding:4px 8px;border-radius:6px;font-weight:900;height:28px;min-width:28px;display:inline-flex;align-items:center;justify-content:center;';
            badge.onclick = showSmartAlerts;
            header.insertBefore(badge, header.firstChild);
        }
        if (badge) {
            if (alerts.length > 0) {
                badge.innerHTML = '🔔' + alerts.length;
                badge.style.display = '';
            } else {
    ...
    setTimeout(updateAlertsBadge, 5000);
...
(function initSalesGoals() {
    ...
    window.setSalesGoal = function() {
        ...
        const html = '<button class="modal-close" onclick="closeModal()">&times;</button>' +
            ...
            '<div style="background:#0D0D0D;border-radius:10px;padding:14px;margin-bottom:12px;">' +
                '<div class="form-group"><label>🎯 الهدف الشهري (ج.م)</label><input type="number" id="goalMonthly" value="' + (goal.monthly || 0) + '" min="0" step="100" /></div>' +
                '<div class="form-group"><label>📅 الهدف اليومي (ج.م)</label><input type="number" id="goalDaily" value="' + (goal.daily || 0) + '" min="0" step="10" /></div>' +
            '</div>' +
            '<button class="btn btn-success btn-block" onclick="saveSalesGoal()">💾 حفظ</button>' +
    ...
    function renderGoalsWidget() {
        ...
        if (!widget) {
            ...
            if (firstStats) firstStats.parentNode.insertBefore(widget, firstStats);
        }
        ...
        widget.innerHTML = 
            ...
            (progress.daily.target > 0 ? 
                '<div style="margin-bottom:10px;">' +
                    ...
                    '<div style="background:#0D0D0D;border-radius:10px;height:8px;overflow:hidden;">' +
                        '<div style="background:' + dailyColor + ';height:100%;width:' + dailyPct + '%;transition:width 0.5s;"></div>' +
                    '</div>' +
            ...
            (progress.monthly.target > 0 ? 
                '<div>' +
                    ...
                    '</div>' +
                    '<div style="background:#0D0D0D;border-radius:10px;height:8px;overflow:hidden;">' +
                        '<div style="background:' + monthlyColor + ';height:100%;width:' + monthlyPct + '%;transition:width 0.5s;"></div>' +
                    '</div>' +
                    '<div style="text-align:left;font-size:10px;color:' + monthlyColor + ';margin-top:2px;font-weight:900;">' + monthlyPct + '%</div>' +
                '</div>' : '');
    }

    window.renderGoalsWidget = renderGoalsWidget;
...
(function initAutoDarkMode() {
    window.enableAutoDarkMode = function() {
        ...
        checkAutoTheme();
        if (typeof showToast === 'function') showToast('✅ الوضع التلقائي مفعّل', 'success');
    };

    window.disableAutoDarkMode = function() {
        localStorage.removeItem('mizan_auto_theme');
        if (typeof showToast === 'function') showToast('⏸️ الوضع التلقائي موقوف', 'info');
    };
...
(function initDiscountCoupons() {
    window.coupons = getData('coupons', [
        { id: 1, code: 'WELCOME10', type: 'percent', value: 10, minAmount: 0, usageLimit: 0, usedCount: 0, active: true },
        { id: 2, code: 'SAVE50', type: 'fixed', value: 50, minAmount: 500, usageLimit: 0, usedCount: 0, active: true }
    ]);
    ...
    window.validateCoupon = function(code, amount) {
        ...
        if (coupon.expiry && new Date(coupon.expiry) < new Date()) return { valid: false, error: '❌ انتهت الصلاحية' };
    ...
    window.showCouponInput = function() {
        ...
        setTimeout(function() {
            ...
            if (input) {
                input.focus();
                input.onkeydown = function(e) { if (e.key === 'Enter') applyCoupon(); };
            }
        }, 200);
    };
    ...
    window.applyCoupon = function() {
        ...
        if (typeof currentSaleItems !== 'undefined' && currentSaleItems.length > 0) {
            ...
            if (typeof showToast === 'function') showToast('🎁 خصم ' + result.discount.toFixed(2) + ' ج.م', 'success');
            if (typeof closeModal === 'function') closeModal();
        } else {
    ...
    window.manageCoupons = function() {
        let html = '<button class="modal-close" onclick="closeModal()">&times;</button>' +
            '<h3>🎁 إدارة القسائم</h3>' +
            '<button class="btn btn-success btn-block" onclick="addNewCoupon()" style="margin-bottom:12px;">➕ إضافة قسيمة</button>';
        ...
        if (coupons.length === 0) {
            ...
            coupons.forEach(function(c) {
                const valueText = c.type === 'percent' ? c.value + '%' : c.value + ' ج.م';
                html += '<div style="background:#0D0D0D;border-radius:10px;padding:12px;margin-bottom:8px;border-right:4px solid ' + (c.active ? '#2D8F5E' : '#5D5D5D') + ';">' +
                    '<div style="display:flex;justify-content:space-between;align-items:center;">' +
                        '<strong style="color:#C9A94E;font-family:monospace;font-size:15px;">' + c.code + '</strong>' +
                    ...
                    '</div>' +
                    '<div style="font-size:11px;color:#A89070;margin-top:4px;">🎯 الحد الأدنى: ' + (c.minAmount || 0) + ' ج.م • استُخدم: ' + (c.usedCount || 0) + '</div>' +
    ...
    window.addNewCoupon = function() {
        ...
        const minAmount = parseFloat(prompt('📊 الحد الأدنى (0 = بدون):', '0')) || 0;

        coupons.push({
            id: Date.now(), code: upperCode,
            type: type === '1' ? 'percent' : 'fixed',
            value: value, minAmount: minAmount,
            usageLimit: 0, usedCount: 0, active: true
        });

        saveCoupons();
        if (typeof showToast === 'function') showToast('✅ تم إضافة: ' + upperCode, 'success');
    ...
    window.deleteCoupon = function(id) {
        if (!confirm('⚠️ حذف القسيمة؟')) return;
        window.coupons = coupons.filter(function(c) { return c.id !== id; });
        saveCoupons();
        manageCoupons();
    };

    // إضافة زر القسيمة في الكاشير - مرة واحدة فقط
    function addCouponButton() {
        if (document.getElementById('couponBtn')) return;
    ...
    function tryAdd() {
        attempts++;
        addCouponButton();
        if (attempts < 10 && !document.getElementById('couponBtn')) {
            setTimeout(tryAdd, 1500);
        }
    }
...
// ═══════════════════════════════════════════════════════════
// 🎯 إضافة أزرار قائمة "المزيد" - مرة واحدة فقط
// ═══════════════════════════════════════════════════════════
(function addExtrasV2Menu() {
    let menuAdded = false;
    ...
    function addMenuItems() {
        ...
        // ✅ التحقق من خلال سمة على عنصر القائمة نفسه
        if (moreMenu.getAttribute('data-extras-v2-added') === 'true') return;
        moreMenu.setAttribute('data-extras-v2-added', 'true');
        menuAdded = true;

        const items = [
            { icon: 'fa-ticket-alt', color: '#9B59B6', label: 'القسائم', action: 'manageCoupons' },
            { icon: 'fa-bullseye', color: '#2D8F5E', label: 'أهداف المبيعات', action: 'setSalesGoal' }
        ];
        ...
        console.log('✅ تم إضافة عناصر قائمة المزيد');
    }
    ...
    function tryAdd() {
        attempts++;
        addMenuItems();
        if (attempts < 10 && !menuAdded) {
            setTimeout(tryAdd, 1500);
        }
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() { setTimeout(tryAdd, 3000); });
    } else {
...
console.log('');
console.log('════════════════════════════════════════════════');
console.log('🎉 تم تحميل app-extras-v2.js (نسخة مصححة)');
console.log('════════════════════════════════════════════════');
